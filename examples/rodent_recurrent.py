    ...
    Concat(
        imitation_target=Sequential([
            ...
            *make_mlp_layers(enc_sizes, rngs, activation, activation_last_layer=False),
            VariationalBottleneck(net_config.latent_size, rngs, net_config.kl_weight,
                                  net_config.latent_min_std),
        ]),
        proprioception=Flattener(),
    ),
    Dense(dec_sizes[0], dec_sizes[1], rngs, activation),
    LSTM(dec_sizes[1], dec_sizes[2], rngs),
    LSTM(dec_sizes[2], dec_sizes[3], rngs),
    Dense(dec_sizes[3], dec_sizes[4], rngs),
])
