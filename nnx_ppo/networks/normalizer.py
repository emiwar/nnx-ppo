...
class Normalizer(StatefulModule):
    ...
    def __call__(self, state, x):
        ...
        std = jax.lax.cond(self.counter[...]>0,
                           self.M2_to_std,
    ...
    def update_statistics(self, last_rollout: Transition, total_steps: jax.Array) -> None:
        ...
        delta_old = jax.tree.map(lambda batch_mean, old: batch_mean-old, batch_mean, self.mean[...])
        self.mean[...] = jax.tree.map(lambda old, delta: old + delta * frac, self.mean[...], delta_old)
        delta_new = jax.tree.map(lambda batch_mean, old: batch_mean-old, batch_mean, self.mean[...])

        batch_var = jax.tree.map(lambda x: jp.var(x, axis=(0, 1)), obs)
        self.M2[...] = jax.tree.map(lambda old, batch_var, delta_old, delta_new: old + batch_count*batch_var + batch_count*delta_old*delta_new,
                                     self.M2[...], batch_var, delta_old, delta_new)
